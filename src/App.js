  ...
  return (
    <div className="background">
      ...
      {weather_data && <div>
        ...
        <div>{forc && <div>
          <div className="forcast">{forc.list.map((weather, i)=>{if (i%8==0){return <div key={i}>{new Date(weather.dt * 1000).toLocaleTimeString([], {hour: "2-digit", minute: "2-digit"})} PM</div>}else
      {return null}})}
          {new Date(weather.dt * 1000).list.main.temp}
      </div>
          </div> }</div>
        </div>
        }
